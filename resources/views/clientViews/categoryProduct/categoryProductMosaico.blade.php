...
@section('content')
    ...
    <div class="row ">
        ...
        <img src="{{$viewMosaics->image_product_1}}" class="card-img-top" alt="...">
        <div class="card-body">
        <h5 class="card-title text-center title-card">{{$viewMosaics->name}}</h5>
        <p class="card-text text-center price-card">R$ {{$viewMosaics->price}} <a href="#" class="a-href-card">no pix</a></p>
        ...
        <a href="{{ route('showProductClient', ['id' => $viewMosaics->id]) }}" class="btn btn-secondary d-flex justify-content-center">Ver mais...</a>
        </div>
        </div><!-- final do card -->
        </div>
        @endforeach
